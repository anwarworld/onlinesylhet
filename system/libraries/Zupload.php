...
//checks for extensions
    ...
    function isSupportedSize() {
        if ($this->_maxfilesize < $this->_currentfilesize)
            return false;
        else
            return true;
    }
    ...
    function upload($multiple = false) { //pass $multiple=true while uploading multiple file. see example at top.	
        ...
        if (!$this->isValidUploadDir()) {
            $this->_uploaddir.="/";
        } //correcting the upload path.	
        if (!file_exists($this->_uploaddir)) {
            mkdir($this->_uploaddir, 0777, true);
        ...
        if (!$this->isSupportedSize()) {
            ...
            if (move_uploaded_file($_FILES[$fileinputname]['tmp_name'], $uploadfile)) {
                ...
                switch ($_FILES[$fileinputname]['error']) {
                    case 'UPLOAD_ERR_FORM_SIZE': $msg = "The file exceeds MAX_FILE_SIZE";
                        break;
                    case 'UPLOAD_ERR_INI_SIZE': $msg = "The file exceeds the upload_max_filesize directive";
                        break;
                    case 'UPLOAD_ERR_NO_FILE': $msg = "The browser didn't send a file";
                        break;
                    case 'UPLOAD_ERR_PARTIAL': $msg = "The browser did not complete the upload";
                        break;
